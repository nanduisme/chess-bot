...
class ChessBoard:
    ...
    def move_piece(self, start, end):
        ...
        # Check if the piece is a Pawn and at the promotion rank
        if isinstance(piece, Pawn) and ((piece.color == WHITE and ex == 7) or (piece.color == BLACK and ex == 0)):
            new_piece = piece.pawn_promotion(ex, ey)  # Get the promoted piece
            if new_piece == piece:  # If the promotion didn't change the piece
                print("Promotion canceled. The pawn remains.")
            else:
                self.board[ex][ey] = new_piece  # Replace the pawn with the new piece
                self.board[sx][sy] = ' '
                return  # Early return after promotion

        if isinstance(piece, Pawn) and abs(sy - ey) == 1 and self.get_piece(end) == ' ':
            if (self.en_passant_target == (ex, ey) and
                (piece.color == WHITE and sx == 4 or piece.color == BLACK and sx == 3)):
                # Capture the pawn that just moved two squares
                self.board[ex][ey] = ' '  # Remove the captured pawn
                print(f"En passant capture at {end}.")
